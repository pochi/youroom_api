...
describe Youroom::DestroyParticipation do
  ...
  describe "#initialize" do
    describe "when can not create instanse" do
      ...
      it do
        lambda { Youroom::DestroyParticipation.new(*@error_data1) }.should raise_exception(ArgumentError)
      end

      it do
        lambda { Youroom::DestroyParticipation.new(*@error_data2) }.should raise_exception(ArgumentError)
      end

    end

    describe "when can create instance" do
      subject { Youroom::DestroyParticipation.new("room_id", "test_pochi@gmail.com") }
  ...
  describe "#params" do
    before do
      @participation = Youroom::DestroyParticipation.new("room_id", "test_pochi@gmail.com")
    end
